...
class Store {
            ...
            liquidityPools: [
                {
                    ...
                    pairSymbol: 'WETH',
                    wethKaniRoi: '',
                    linkLiqui: 'https://pools.balancer.exchange/#/pool/0x8B2E66C3B277b2086a976d053f1762119A92D280/',
                    linkTrade: 'https://legacy.balancer.exchange/#/swap/0x790aCe920bAF3af2b773D4556A69490e077F6B4A',
                ...
                },
                {
                    ...
                    pairSymbol: 'USDT',
                    usdtKaniRoi: '',
                    linkLiqui: 'https://app.uniswap.org/#/add/0xdAC17F958D2ee523a2206206994597C13D831ec7/0x790aCe920bAF3af2b773D4556A69490e077F6B4A',
                    linkTrade: 'https://app.uniswap.org/#/swap?inputCurrency=0xdac17f958d2ee523a2206206994597c13d831ec7&outputCurrency=0x790aCe920bAF3af2b773D4556A69490e077F6B4A',
                    depositsEnabled: true,
                    tokens: [
                        {
                            id: 'uni',
    ...
    _checkApproval = async (asset, account, amount, contract, callback) => {
        try {
            ...
            const allowance = await erc20USDTContract.methods.allowance(account.address, contract).call({from: account.address})
    ...
    _checkApprovalWaitForConfirmation = async (asset, account, amount, contract, callback) => {
        ...
        if (parseFloat(ethAllowance) < parseFloat(amount)) {
            erc20Contract.methods.approve(contract, web3.utils.toWei("999999999999999", "ether")).send({
                from: account.address,
                gasPrice: web3.utils.toWei(await this._getGasPrice(), 'gwei')
            })
    ...
    _getstakedBalance = async (web3, asset, account, callback) => {
        ...
        try {
            if( asset.id === "eth" || asset.id === 'bpt' || asset.id === 'uni'){
                ...
                var balanceObj = await erc20Contract.methods.plyr_(account.address).call({from: account.address});
                var balance = balanceObj.stake;
            } else{
                var balance = await erc20Contract.methods.balanceOf(asset.id,account.address).call({from: account.address});
            }
            balance = parseFloat(balance) / 10 ** asset.decimals
            callback(null, parseFloat(balance))
        } catch (ex) {
    ...
    _getRewardsAvailable = async (web3, asset, account, callback) => {
        ...
        if(asset.id === 'kani'){
            try {
                var earned = await erc20Contract.methods.cal_out(account.address).call({from: account.address});
    ...
    _callBanlanceKaniStake = async (asset, account, amount, callback) => {
        ...
        const ethContract = new web3.eth.Contract(asset.rewardsABI, asset.rewardsAddress)
        ...
        ethContract.methods.stake(amountToSend).send({ from: account.address})
            .on('transactionHash', function(hash){
                console.log(hash)
                callback(null, hash)
            })
    ...
    _callKaniStake = async (asset, account, amount, callback) => {
        ...
        ethContract.methods.deposit(amountToSend).send({ from: account.address})
            .on('transactionHash', function(hash){
                console.log(hash)
                callback(null, hash)
            })
  ...
  _callStake = async (asset, account, amount, callback) => {
    ...
    ethContract.methods.stake(asset.id,amountToSend).send({ from: account.address})
      .on('transactionHash', function(hash){
        console.log(hash);
          callback(null, hash)
  ...
  withdraw = (payload) => {
    ...
    _callKaniWithdraw = async (asset, account, amount, callback) => {
        ...
        yCurveFiContract.methods.withdraw(amountToSend).send({ from: account.address})
            .on('transactionHash', function(hash){
                console.log(hash)
                callback(null, hash)
            })
  ...
  _callWithdraw = async (asset, account, amount, callback) => {
    ...
    yCurveFiContract.methods.withdraw(asset.id,amountToSend).send({ from: account.address})
      .on('transactionHash', function(hash){
        console.log(hash)
        callback(null, hash)
      })
  ...
  getReward = (payload) => {
    ...
    _callKaniGetReward = async (asset, account, callback) => {
        ...
        yCurveFiContract.methods.claim().send({ from: account.address })
            .on('transactionHash', function(hash){
                console.log(hash)
                callback(null, hash)
            })
  ...
  _callGetReward = async (asset, account, callback) => {
    ...
    yCurveFiContract.methods.getReward().send({ from: account.address })
      .on('transactionHash', function(hash){
        console.log(hash)
        callback(null, hash)
      })
  ...
  _callExit = async (asset, account, callback) => {
    ...
    yCurveFiContract.methods.exit().send({ from: account.address})
      .on('transactionHash', function(hash){
        console.log(hash)
        callback(null, hash)
      })
  ...
  getGovernanceV2Requirements = async (payload) => {
    ...
    getRoi = async (payload) => {
        ...
        const bptContract =  new web3.eth.Contract(config.bpoolABI,config.bpoolAddress);
        const kaniPrice = await bptContract.methods.getSpotPrice(weth, kani).call({from: account.address});
        // const kaniPriceWei = web3.utils.fromWei(kaniPrice, "ether")
        // console.log("kaniPrice-----",kaniPriceWei)

        const totalKaniAmount = await bptContract.methods.getBalance(kani).call({from: account.address});

        const totalBptAmount = await bptContract.methods.totalSupply().call({from: account.address});

        const totalWethAmount = await bptContract.methods.getBalance(weth).call({from: account.address});

        var kPrice = web3.utils.fromWei(kaniPrice, "ether");
        const bptPrice = totalKaniAmount / totalBptAmount * kPrice + totalWethAmount / totalBptAmount;
        ...
        const pool2 = new web3.eth.Contract(config.balanceKaniABI,config.balanceKaniAddress);
        const weeklyReward = await pool2.methods.initreward().call({from: account.address});

        const totalBptStaked = await pool2.methods.totalSupply().call({from: account.address});
        // const totalBptStaked = totalBptAmount;

        console.log("weekly reward:"+web3.utils.fromWei(weeklyReward, "ether")+" totalBptStaked:");
        // console.log("total stake --------",totalBptStaked)

        if (totalBptStaked > 0) {
            const yearROI = parseFloat((weeklyReward / totalBptStaked) * kPrice / bptPrice * 52*100).toFixed(2)+ "%";
    ...
    getStableRoi = async (payload) => {
        ...
        const bptContract =  new web3.eth.Contract(config.bpoolABI,config.bpoolAddress);
        const stableContract =  new web3.eth.Contract(config.bpoolABI,config.governanceV2DAIAddress);
        const kaniPrice = await bptContract.methods.getSpotPrice(weth, kani).call({from: account.address});

        const mUSDWETHContract =  new web3.eth.Contract(config.bpoolABI,config.mUSDWETHAddress);

        const wethPrice = await mUSDWETHContract.methods.getSpotPrice(mUSD, weth).call({from: account.address});
        ...
        const totalDAIAmount = await stableContract.methods.totalSupply().call({from: account.address});
        var daiAmount = web3.utils.fromWei(totalDAIAmount, "ether");

        // console.log("---------totalDAIAmount",web3.utils.fromWei(totalDAIAmount, "ether"));
        const yearROI = parseFloat((5000*kPrice*wPrice / daiAmount) *100* 365).toFixed(2)+ "%";
    ...
    getKaniRoi = async (payload) => {
        ...
        const govern = new web3.eth.Contract(config.governanceKaniABI,config.governanceKani);
        const dailyReward = 500000/365;
        const global = await govern.methods.global_(0).call({from: account.address});
        const totalStake = web3.utils.fromWei(global.total_stake,"ether");

        const yearROI = parseFloat(dailyReward / totalStake * 365 * 100).toFixed(2) + "%";
    ...
    getPool2RoiOfUniswap = async (payload) => {
        ...
        const uniswapContract =  new web3.eth.Contract(config.uniswapABI,config.uniAddress);

        const reservers = await uniswapContract.methods.getReserves().call({from: account.address});
        const totalKaniAmount = web3.utils.fromWei(reservers._reserve0, "ether");
        const totalUsdtAmount = reservers._reserve1/1000000;
        const totalUniAmount = web3.utils.fromWei(await uniswapContract.methods.totalSupply().call({from: account.address}), "ether");

        const kaniPrice = totalKaniAmount / totalUsdtAmount;
        const uniPrice = totalKaniAmount / totalUniAmount * kaniPrice + totalUsdtAmount / totalUniAmount;

        // pool2 contract address : 0x4fed7a096243bc7da8940835fe1a7935dace2f89
        const pool2 = new web3.eth.Contract(config.uniswapKaniABI,config.uniswapKaniAddress);
        const weeklyReward = await pool2.methods.initreward().call({from: account.address});
        const totalUniStaked = await pool2.methods.totalSupply().call({from: account.address});
        if (totalUniStaked > 0) {
            const yearROI = parseFloat((weeklyReward / totalUniStaked) * (kaniPrice / uniPrice) * 52*100).toFixed(2)+ "%";
